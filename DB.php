...
global $dbCon;
  ...
  class DB
  {
	...
	var $link, $server,$user,$pass,$base,$force = false;

    /** Connect to a MySQL database to be able to use the methods below.
      */
    function __construct( $base, $server, $user, $pass, $force = false ) { 
    ...
    {	
		//if ( defined( "noMySQL" ) && $this->force == false ) return false;
		$this -> GetCon ();
      $this->nbQueries++;
	  
	  $found = false;
	  
	  $memCacheSettings = array( 'type' => 'sql-query', $query );
	  
	  $memcacheOn = false;
	  
	  //if ( defined( 'MEMCACHE_ON' ) ) $memcacheOn = MEMCACHE_ON;
	  
	  if ( $memcacheOn == true ) {
		  
		 $MemcacheStuff = new MemcacheStuff;
		 
		 if ( $s = $MemcacheStuff -> GetPage ( $memCacheSettings ) ) {
	  ...
	  if ( $found == false ) {
		  ...
		  $this->lastResult = $this->link->query($query) or $this->debugAndDie($query);
	
		  //$this->debug($debug, $query, $this->lastResult);
		  
		  if ( $memcacheOn == true ) {
			  
			 $MemcacheStuff -> SavePage ( $memCacheSettings, $s, true, 30 ); 
    ...
    function execute($query, $debug = -1)
    {
		...
		$this -> GetCon ();
      $this->nbQueries++;
      $this->link->query($query) or $this->debugAndDie($query);
	...
	function fetchNextArray($result = NULL)
    {
	  ...
	  if ($result == NULL)
        $result = $this->lastResult;
		if ( is_string( $result ) ) throw new exception("String passed to num rows? - " . $result);
      if ($result == NULL || mysqli_num_rows($result) < 1)
        return NULL;
      else
	...
	function fieldName($result = NULL, $field = 0)
    {
      if ($result == NULL)
        $result = $this->lastResult;
      ...
      return !empty($fieldInfo) ? $fieldInfo->name : NULL;
    }
    ...
    function queryUniqueObject($query, $debug = -1)
    {
      ...
      if ( strpos( $query, 'LIMIT ' ) === false )  $query .= " LIMIT 1";

      $this->nbQueries++;
      $result = $this->link->query($query) or $this->debugAndDie($query);
    ...
    function queryUniqueValue($query, $debug = -1)
    {
      ...
      return !empty( $line[0] ) ? $line[0] : null;
    }
